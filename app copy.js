...
function searchDescendants (filteredName, people) {
	...
	for (var i = 0; i <people.length; i++) {
		for (filteredName.id in people[i])
			filteredNameKid.pop(people[i]).id;
			allDescendants.pop(filteredNameKids);
		} if (filteredNameKids[i].id === 0) {
				return allDescendants
			} else return searchDescendants;
				alert(filteredNameKids);	
}
